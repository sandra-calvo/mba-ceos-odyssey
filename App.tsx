...
const StartScreenImage: React.FC = () => (
    <svg width="100%" height="100%" viewBox="0 0 400 300" className="mx-auto my-4 max-h-[300px]">
        <defs>
...
const CharacterSelectionScreen: React.FC<{ 
    ...
    return (
        <div className="w-full h-full min-h-screen flex items-center justify-center bg-[#202020] text-black font-['Press_Start_2P'] p-4 md:p-8 overflow-y-auto">
            <div className="w-full max-w-[1200px] flex flex-col md:flex-row bg-[#e6e6e6] shadow-[15px_15px_0px_0px_rgba(0,0,0,0.5)] relative mt-8 md:mt-0 border-4 border-black">
                
                {/* Header Tape */}
                <div className="absolute -top-6 left-1/2 -translate-x-1/2 bg-yellow-400 border-2 border-black px-4 py-2 transform -rotate-1 z-10 shadow-md whitespace-nowrap">
                   <h2 className="text-sm md:text-xl lg:text-2xl text-black">THE CEO'S ODYSSEY</h2>
                </div>

                {/* --- LEFT COL: Character Selection --- */}
                <div className="w-full md:w-1/4 flex flex-row md:flex-col items-center justify-center border-b-4 md:border-b-0 md:border-r-4 border-black p-4 gap-4 md:gap-0 md:space-y-6 bg-[#d4d4d4]">
                    <div className="w-full text-center hidden md:block">
                        <h3 className="text-xs md:text-sm mb-2 underline decoration-2 underline-offset-4">SELECT CEO</h3>
                    </div>
                    
                    {/* Pedro Option */}
                    <div 
                        onClick={() => onSelect('pedro')}
                        className={`group cursor-pointer relative p-2 md:p-4 border-4 transition-all duration-200 flex-1 md:flex-none w-full ${selected === 'pedro' ? 'border-blue-600 bg-white shadow-[2px_2px_0px_0px_rgba(37,99,235,1)] md:scale-105 md:shadow-[4px_4px_0px_0px_rgba(37,99,235,1)]' : 'border-gray-500 bg-gray-200 hover:bg-gray-100 hover:border-gray-600'}`}
                    >
                        <div className="h-[60px] md:h-[100px] flex items-center justify-center relative overflow-hidden">
                             {/* Large Portrait Pedro */}
                             <svg width="60" height="80" viewBox="0 0 60 80" className="transform scale-75 md:scale-100">
                        ...
                        <p className="text-center mt-2 text-[8px] md:text-xs font-bold">PEDRO</p>
                    ...
                    <div 
                        ...
                        className={`group cursor-pointer relative p-2 md:p-4 border-4 transition-all duration-200 flex-1 md:flex-none w-full ${selected === 'sandra' ? 'border-blue-600 bg-white shadow-[2px_2px_0px_0px_rgba(37,99,235,1)] md:scale-105 md:shadow-[4px_4px_0px_0px_rgba(37,99,235,1)]' : 'border-gray-500 bg-gray-200 hover:bg-gray-100 hover:border-gray-600'}`}
                    >
                        <div className="h-[60px] md:h-[100px] flex items-center justify-center relative overflow-hidden">
                        ...
                        <p className="text-center mt-2 text-[8px] md:text-xs font-bold">SANDRA</p>
                ...
                <div className="flex-1 p-4 md:p-8 bg-[#fdfbf7] flex flex-col relative overflow-hidden min-h-[300px]">
                    ...
                    <div className="border-4 border-black p-4 md:p-6 w-full h-full bg-white shadow-lg flex flex-col relative">
                        {/* Stamp */}
                        <div className="absolute top-10 md:top-20 right-4 md:right-8 border-4 border-red-700 p-2 transform -rotate-12 opacity-70 pointer-events-none mix-blend-multiply" style={{ borderStyle: 'double', borderWidth: '6px' }}>
                             <p className="text-red-700 text-center font-black text-[8px] md:text-sm tracking-widest uppercase leading-tight font-sans">
                                CONFIDENTIAL<br/>
                                <span className="text-sm md:text-xl block mt-1">TOP SECRET</span>
                            </p>
                        </div>

                        <h3 className="text-center text-sm md:text-xl font-bold mb-4 md:mb-6 font-serif border-b-2 border-black pb-2">EMPLOYMENT AGREEMENT</h3>
                        
                        <div className="flex-1 space-y-2 md:space-y-4 text-[8px] md:text-xs leading-4 md:leading-6 text-gray-800 font-mono overflow-y-auto">
                            <div className="flex justify-between">
                                <span><strong>DATE:</strong> TODAY</span>
                                <span><strong>REF:</strong> #CEO-001</span>
                            </div>
                            <p><strong>PARTIES:</strong></p>
                            <p>1. The Board of Directors of Aalto Industries ("Overlords")</p>
                            ...
                            <div className="space-y-2 md:space-y-3">
                                ...
                                <p>You are taking over Aalto Industries at a crossroads. We expect results, and we expect them fast.</p>
                        ...
                        <div className="mt-4 pt-4 flex flex-col items-center justify-end bg-gray-50 border-t-2 border-black">
                            <p className="text-[8px] md:text-[10px] text-gray-500 mb-2 tracking-widest uppercase">DIGITAL SIGNATURE REQUIRED</p>
                            <button 
                                onClick={onConfirm}
                                className="group relative bg-yellow-400 text-black border-4 border-black px-6 py-3 md:px-8 md:py-4 text-xs md:text-sm font-bold shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] active:translate-y-1 active:shadow-none transition-all hover:bg-yellow-300"
                            >
                ...
                {/* --- RIGHT COL: Building (Hidden on Mobile to save space) --- */}
                <div className="hidden md:flex w-1/4 border-l-4 border-black bg-gradient-to-b from-blue-300 to-blue-500 relative overflow-hidden flex-col justify-end">
...
const TouchControls = ({ onInput }: { onInput: (key: string, active: boolean) => void }) => {
    // Prevent default context menu on long press and handling mouse events for testing
    const handlers = (key: string) => ({
        ...
        onMouseDown: (e: React.MouseEvent) => { e.preventDefault(); onInput(key, true); },
        onMouseUp: (e: React.MouseEvent) => { e.preventDefault(); onInput(key, false); },
        onMouseLeave: () => { onInput(key, false); }
    });

    return (
        <div className="fixed bottom-6 left-0 right-0 px-6 flex justify-between items-end pointer-events-none z-40 select-none">
            {/* D-Pad */}
            <div className="flex gap-4 pointer-events-auto">
                <button {...handlers('ArrowLeft')} className="w-16 h-16 md:w-20 md:h-20 bg-white/10 border-2 border-white/40 rounded-full text-white text-2xl backdrop-blur-sm active:bg-white/30 flex items-center justify-center">←</button>
                <button {...handlers('ArrowRight')} className="w-16 h-16 md:w-20 md:h-20 bg-white/10 border-2 border-white/40 rounded-full text-white text-2xl backdrop-blur-sm active:bg-white/30 flex items-center justify-center">→</button>
            </div>
            
            {/* Jump Button */}
            <div className="pointer-events-auto">
                 <button {...handlers('ArrowUp')} className="w-20 h-20 md:w-24 md:h-24 bg-yellow-400/20 border-2 border-yellow-400/50 rounded-full text-yellow-400 font-bold text-xs backdrop-blur-sm active:bg-yellow-400/40 flex items-center justify-center shadow-[0_0_15px_rgba(250,204,21,0.3)]">JUMP</button>
            </div>
        </div>
    )
}
...
const App: React.FC = () => {
    ...
    useEffect(() => {
        const handleResize = () => {
            ...
            // We use a slightly smaller factor to ensure no scrollbars/cutoffs
    ...
    const renderGameContent = () => {
        if (showInstructions) {
            return (
                <div className="w-full h-screen flex items-center justify-center bg-[#0c0a1a] text-white p-4">
                    <div className="w-full max-w-2xl bg-[#1e223b] border-4 border-white p-4 md:p-8 text-center flex flex-col items-center shadow-lg">
                        <h1 className="text-xl md:text-4xl text-yellow-400 mb-2 font-bold tracking-widest uppercase leading-normal">The CEO's Odyssey</h1>
                        <p className="text-sm md:text-lg text-gray-300 mb-4 tracking-wider">Trust is your currency</p>
                        ...
                        <button 
                            ...
                            className="mt-6 text-lg md:text-2xl text-green-400 hover:text-green-300 animate-blink transition-colors duration-200"
                        >
                            &gt; PRESS START &lt;
                        </button>

                        <p className="mt-8 text-[10px] md:text-xs text-gray-500">@2025 Sandra Calvo</p>
                    </div>
                </div>
            );
        }
        ...
        if (gameState === 'performanceReview') {
            return (
                <div className="absolute inset-0 bg-black bg-opacity-75 flex items-center justify-center p-4 z-50">
                    <div className="w-full max-w-2xl bg-[#1e223b] text-white border-4 border-white p-6 shadow-lg overflow-y-auto max-h-[90vh]">
                        <h2 className="text-lg md:text-2xl mb-4 text-yellow-400">The CEO's Odyssey: Performance Review</h2>
        ...
        if (gameState === 'keyTakeaways') {
             return (
                <div className="absolute inset-0 bg-black bg-opacity-75 flex items-center justify-center p-4 z-50">
                    <div className="w-full max-w-4xl bg-[#1e223b] text-white border-4 border-white p-4 md:p-6 shadow-lg h-[80vh] flex flex-col">
                        <h2 className="text-lg md:text-2xl mb-6 text-yellow-400 uppercase text-center border-b-2 border-white pb-2">10 Strategic Takeaways</h2>
            ...
            return (
               <div className="absolute inset-0 bg-black bg-opacity-75 flex items-center justify-center p-4 z-50">
                   <div className="w-full max-w-2xl bg-[#1e223b] text-white border-4 border-white p-6 shadow-lg text-center flex flex-col items-center">
                       <h1 className="text-xl md:text-3xl text-yellow-400 mb-8 uppercase tracking-widest leading-relaxed">
        ...
        return (
            <div className="w-full h-screen flex items-center justify-center bg-black relative overflow-hidden">
                ...
                {/* --- UI LAYER (Unscaled / Viewport Relative) --- */}

                {/* On-Screen Controls for Mobile */}
                <TouchControls onInput={handleTouchInput} />

                {/* Modal Overlay - Rendered outside scaled div for responsiveness */}
